...
reader = easyocr.Reader(['ko'], recog_network='custom_1st') 
...
def export_csv(results):
    with open('trained_1st.csv', 'w', encoding='cp949') as f:
...
for test_image in test_data:
    ...
    for result in results:
        ...
        cv2.imwrite(f"result_data_plate_1st/{test_image.split('/')[-1]}", orig_img)
